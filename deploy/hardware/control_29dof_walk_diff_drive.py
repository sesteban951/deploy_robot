##
#
# Control node for 29DoF MjLab differential-drive upright WALKING (G1-Standing-DiffDrive), HARDWARE.
#
##
...
class ControlNode(Node):
    ...
    (G1-Standing-DiffDrive) on hardware.
    ...
    turn-in-place (with hysteresis and a dwell on band changes), and the live stick
    range maps onto the trained band -- forward vx 0.50..1.00, backward -0.40..-0.90,
    turn |wz| 0.50..1.50. Nothing in the gaps and never a mixed arc.
    ...
    A missing joystick commands a zero twist (stand), never an autonomous walk.
    """
    ...
    def commanded_twist(self):
        # hardware safety: no joystick -> centered sticks -> idle (stand). Routed through the
        # shaper so a dropped joystick stops the robot immediately (stops are never dwelled).
    ...
    def control_callback(self):
        ...
        # at phase 0, in idle mode -- no stale dwell timer or mode from an earlier session.
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description='Asynchronous Control Node for the MjLab differential-drive walking policy (hardware).'
    )
