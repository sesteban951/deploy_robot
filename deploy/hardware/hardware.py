...
class ControlNode(Node):
    def __init__(self, config_path: str,
        ...
        # waist pitch sign flip between the motor frame and the model frame (off by default).
        # applied to readings (q, dq, ddq, tau_est) and commands (q, dq, tau), but not to gains.
        self.flip_waist_pitch = flip_waist_pitch
        self.joint_sign = np.ones(G1_NUM_MOTOR)
        if self.flip_waist_pitch:
            self.joint_sign[G1JointIndex.WaistPitch] = -1.0
            print("WARNING: waist pitch sign flip is ENABLED.")
    ...
    def Init(self):
        ...
        # latched hardware info: which flags this run used (recorded by the logger)
        self.hardware_info_pub = self.create_publisher(String, HARDWARE_INFO_TOPIC, experiment_info_qos())
        hardware_info_msg = String()
        hardware_info_msg.data = json.dumps({"flip_waist_pitch": self.flip_waist_pitch})
