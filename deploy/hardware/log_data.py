...
# 29dof G1 hardware vector sizes
G1_NUM_MOTOR = 29
...
FSM_TIME_N    = 1                  # [fsm_time]

# start logging only while in this FSM state
...
class LogNode(Node):
    """
    Asynchronous logging node that subscribes to hardware topics and writes
    ...
    def __init__(self, output_path: str, log_freq: float, dump_period: float):
        ...
        # current FSM state (only log while this equals TARGET_FSM_STATE)
        self.fsm_state = "init"
        ...
        self.fsm_time_logger   = Logger(output_path, FSM_TIME_N,    dataset_name="fsm_time")
        ...
        self.fsm_time_sub   = self.create_subscription(Float64,           'deploy_robot/fsm_time',         self.fsm_time_callback,   10)
    ...
    def fsm_time_callback(self, msg: Float64):
        self.latest_fsm_time = np.array([msg.data], dtype=np.float32)
    ...
    def log_callback(self):
        ...
        self.fsm_time_logger.log(self.latest_fsm_time)
...
def default_output_path() -> str:
    ...
    logs_dir = os.path.join(ROOT_DIR, "logs", "hardware") if ROOT_DIR else os.path.join("logs", "hardware")
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description='Asynchronous data logging node.'
    )
    parser.add_argument(
        '--output',
        type=str,
