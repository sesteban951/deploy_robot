##
#
# Control node for running a 23DoF MjLab velocity tracking policy on the
# 29DoF G1 simulation. The policy's 23 joints are a subset of the 29dof
# hardware joints; the 6 excluded joints (waist roll/pitch, L/R wrist
# pitch/yaw) are held at their default position with the full Kp/Kd.
#
##
...
# 20 (L_wrist_pitch), 21 (L_wrist_yaw), 27 (R_wrist_pitch), 28 (R_wrist_yaw).
...
class ControlNode(Node):
    """
    Asynchronous control node that runs a 23dof policy on a 29dof robot and
    sends actions to the simulation.
    """
    ...
    def init_policy(self):
        ...
        self.policy_joint_indices = np.array(POLICY_JOINT_INDICES, dtype=np.int64)
        ...
        # PD gains (full 29dof, published straight through to the sim)
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to the Mujoco config yaml file. Example: "g1_23to29dof_vel.yaml".'
    )
